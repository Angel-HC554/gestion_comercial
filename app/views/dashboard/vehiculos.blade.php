...
<div class="min-h-screen p-6 md:p-10 space-y-8" x-data="{ activeTab: 'taller' }">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5 space-y-3 cursor-pointer hover:border-emerald-400 transition-colors" @click="activeTab = 'taller'">
            ...
            <p class="text-sm text-zinc-500 leading-relaxed">
                @if($vehiculosEnTallerCount) 
                    Desde <span class="font-semibold text-zinc-900">{{ \Carbon\Carbon::parse($vehiculosEnTaller->first()['fecha_ingreso'])->translatedFormat('d M') }}</span>
                @else 
                    Ninguno en taller.
                @endif
            </p>
        </div>
        <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5 space-y-3 cursor-pointer hover:border-amber-400 transition-colors" @click="activeTab = 'mantenimiento'">
            ...
            <p class="text-sm text-zinc-500">Atención a <strong class="text-amber-600">amarillos</strong> o <strong class="text-red-600">rojos</strong>.</p>
        </div>
        <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5 space-y-3 cursor-pointer hover:border-red-400 transition-colors" @click="activeTab = 'siniestros'">
        ...
        <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm p-5 space-y-3 cursor-pointer hover:border-purple-400 transition-colors" @click="activeTab = 'ordenes500'">
    ...
    <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden mt-8">
        ...
        <div class="flex overflow-x-auto border-b border-zinc-200 bg-zinc-50">
            <button @click="activeTab = 'taller'" 
                :class="activeTab === 'taller' ? 'border-b-2 border-emerald-600 text-emerald-700 bg-white' : 'text-zinc-500 hover:text-zinc-700 hover:bg-zinc-100'"
            ...
            <button @click="activeTab = 'mantenimiento'" 
                :class="activeTab === 'mantenimiento' ? 'border-b-2 border-amber-500 text-amber-700 bg-white' : 'text-zinc-500 hover:text-zinc-700 hover:bg-zinc-100'"
                class="px-6 py-4 text-sm font-bold uppercase tracking-wider transition-colors whitespace-nowrap outline-none">
                Mantenimientos Próximos ({{ $proximosMantenimientoCount }})
            </button>
            <button @click="activeTab = 'siniestros'" 
                :class="activeTab === 'siniestros' ? 'border-b-2 border-red-500 text-red-700 bg-white' : 'text-zinc-500 hover:text-zinc-700 hover:bg-zinc-100'"
                class="px-6 py-4 text-sm font-bold uppercase tracking-wider transition-colors whitespace-nowrap outline-none">
                Siniestros / Golpes ({{ $siniestros->count() }})
            </button>
            <button @click="activeTab = 'ordenes500'" 
                :class="activeTab === 'ordenes500' ? 'border-b-2 border-purple-500 text-purple-700 bg-white' : 'text-zinc-500 hover:text-zinc-700 hover:bg-zinc-100'"
                class="px-6 py-4 text-sm font-bold uppercase tracking-wider transition-colors whitespace-nowrap outline-none">
                        ...
                        @forelse($vehiculosEnTaller as $vehiculo)
                            <tr class="hover:bg-zinc-50">
                                <td class="px-6 py-4">
                                    ...
                                    <p class="text-xs text-zinc-500">{{ $vehiculo['placas'] }} · {{ $vehiculo['marca'] }}</p>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <span class="px-2.5 py-0.5 rounded-full text-xs font-bold {{ $vehiculo['dias_taller'] > 15 ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700' }}">
                                            {{ $vehiculo['dias_taller'] }} días
                                        </span>
                                        <span class="text-xs text-zinc-400">({{ \Carbon\Carbon::parse($vehiculo['fecha_ingreso'])->format('d/m/Y') }})</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-zinc-600 max-w-xs truncate" title="{{ $vehiculo['observacion'] }}">
                                    {{ $vehiculo['observacion'] ?: 'Sin observaciones' }}
                                </td>
                                <td class="px-6 py-4 text-right">
                                    @if($vehiculo['link'])
                                        <a href="{{ $vehiculo['link'] }}" class="text-emerald-600 font-bold hover:underline">Revisar</a>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="6" class="px-6 py-10 text-center text-zinc-500">Ningún vehículo en taller en este proceso.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
                            ...
                            <tr class="hover:bg-zinc-50">
                                ...
                                <td class="px-6 py-4">
                                    @if(in_array($vehiculo['estado'], ['rojo', 'rojo_pasado']))
                                        <span class="px-2.5 py-0.5 rounded-md text-xs font-bold bg-red-100 text-red-700 uppercase">Urgente / Vencido</span>
                                    @else
                                        <span class="px-2.5 py-0.5 rounded-md text-xs font-bold bg-amber-100 text-amber-700 uppercase">Próximo</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-zinc-600">
                                    @if($vehiculo['km_faltantes'] <= 0)
                                        <span class="text-red-600 font-semibold">Excedido por {{ number_format(abs($vehiculo['km_faltantes'])) }} km</span>
                                    @elseif($vehiculo['km_faltantes'] <= 2000)
                                        Faltan {{ number_format($vehiculo['km_faltantes']) }} km
                                    @endif
                                    
                                    @if($vehiculo['dias_restantes'] !== null)
                                        @if($vehiculo['km_faltantes'] <= 2000) <br> <span class="text-xs text-zinc-400">o</span> <br> @endif
                                        @if($vehiculo['dias_restantes'] < 0)
                                            <span class="text-red-600 font-semibold">Vencido hace {{ abs($vehiculo['dias_restantes']) }} días</span>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ $vehiculo['link'] }}" class="text-amber-600 font-bold hover:underline">Agendar</a>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="px-6 py-10 text-center text-zinc-500">Ningún vehículo requiere atención en este proceso.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
                ...
                <table class="min-w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-zinc-100">
                        @forelse($siniestros as $siniestro)
                            <tr class="hover:bg-zinc-50">
                                <td class="px-6 py-4">
                                    <p class="font-bold text-zinc-900">{{ \Carbon\Carbon::parse($siniestro->fecha)->translatedFormat('d M Y') }}</p>
                                    <p class="text-xs font-semibold {{ str_contains($siniestro->tipo, 'Semanal') ? 'text-purple-600' : 'text-blue-600' }}">{{ $siniestro->tipo }}</p>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <p class="font-medium text-zinc-700">{{ $siniestro->vehiculo?->departamento ?? 'N/A' }}</p>
                                </td>
                                <td class="px-6 py-4 text-zinc-700 max-w-sm">{{ $siniestro->detalles }}</td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex flex-col items-end gap-1">
                                        @if($siniestro->link_evidencia)
                                            <a href="{{ $siniestro->link_evidencia }}" target="_blank" class="text-xs bg-zinc-100 border border-zinc-300 px-2 py-1 rounded text-zinc-700 font-bold hover:bg-zinc-200">Ver Foto</a>
                                        @else
                                            <span class="text-xs text-zinc-400">Sin foto</span>
                                        @endif
                                        
                                        @if($siniestro->vehiculo)
                                            <a href="/vehiculos/{{ $siniestro->vehiculo->id }}" class="text-red-600 font-bold text-xs hover:underline mt-1">Ir a vehículo</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="5" class="px-6 py-10 text-center text-zinc-500">No hay siniestros reportados en este proceso.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
                    ...
                    <tbody class="divide-y divide-zinc-100">
                        @forelse($ordenes500 as $orden)
                            <tr class="hover:bg-zinc-50">
                                ...
                                <td class="px-6 py-4 font-medium text-zinc-700">{{ $orden->created_at->format('d/m/Y') }}</td>
                                <td class="px-6 py-4">
                                    <p class="font-bold text-zinc-900">{{ $orden->noeconomico }}</p>
                                    <p class="text-xs text-zinc-500">{{ $orden->marca }} · {{ $orden->placas }}</p>
                                </td>
                                <td class="px-6 py-4">
                                    <p class="font-medium text-zinc-700">{{ $orden->vehiculo?->departamento ?? 'N/A' }}</p>
                                    <p class="text-xs text-zinc-500">{{ $orden->vehiculo?->rpe_responsable ?? '' }}</p>
                                </td>
                                <td class="px-6 py-4">
                                    @if($orden->status === 'TERMINADO')
                                        <span class="px-2.5 py-0.5 rounded-md text-xs font-bold bg-emerald-100 text-emerald-800 border border-emerald-200">{{ $orden->status }}</span>
                                    @else
                                        <span class="px-2.5 py-0.5 rounded-md text-xs font-bold bg-amber-100 text-amber-800 border border-amber-200">{{ $orden->status }}</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-right">
                                    @if($orden->vehiculo)
                                        <a href="/vehiculos/{{ $orden->vehiculo->id }}" class="text-purple-600 font-bold hover:underline">Ver</a>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="6" class="px-6 py-10 text-center text-zinc-500">No hay órdenes 500 registradas en este proceso.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
