@extends('layouts.app-layout', ['title' => 'Supervisión Diaria - Matriz'])
    ...
    <div class="min-h-screen pb-10">
        <div class="flex h-full flex-1 flex-col gap-4 mx-6 md:mx-10 pt-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h1 class="text-3xl font-bold tracking-tight text-zinc-900">Matriz de Supervisión Diaria</h1>
            ...
            <form method="GET" id="form-filtros" class="w-full">
                ...
                <div
                    class="flex flex-col justify-center md:flex-row gap-4 items-end md:items-center bg-white rounded-lg border-t-4 border-t-emerald-600 shadow-sm p-4 border border-zinc-200">
            ...
            <div id="contenido-tabla" style="display:none;"
                class="tabla-scrollable bg-white rounded-lg shadow-md border border-zinc-200">
                <table class="tabla-matriz">
                    <thead>
                        <tr>
                            <th class="bg-zinc-100 text-zinc-700 font-bold uppercase text-xs tracking-wider">Departamento /
                                Ubicación</th>
                        ...
                        @forelse($vehiculos as $vehiculo)
                            <tr class="hover:bg-zinc-50 transition-colors">
                                ...
                                @foreach ($vehiculo->status_dias as $status)
                                    <td class="p-1">
                                        @if ($status == 'cumplido')
                                            <a href="{{ optional($vehiculo->latestSupervision)->escaneo_url }}"
                                                target="_blank" class="flex justify-center" title="Cumplido">
                                                <svg class="w-6 h-6 text-emerald-500" fill="currentColor"
                                                    viewBox="0 0 20 20">
                                        ...
                                        @elseif($status == 'taller')
                                            <div class="flex justify-center" title="En Taller">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                                    fill="currentColor" class="w-6 h-6 text-amber-400">
                                                    <path fill-rule="evenodd"
                                        ...
                                        @elseif($status == 'no_cumplido')
                                            <a class="flex justify-center" 
                                                href="/vehiculos/{{ $vehiculo->id }}?tab=diaria"
                                                title="Ir a registrar supervisión faltante">
                                                <svg class="w-6 h-6 text-red-500 hover:text-red-700" fill="currentColor" viewBox="0 0 20 20">
                                                    <path fill-rule="evenodd"
